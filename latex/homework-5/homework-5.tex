...
self-describing.
These are measured in the same unit as the values returned by Python's
\texttt{time.clock}.

% TBD: insert the generated table here

The average number of iterations needed seems to grow linearly with the number
of rows and that holds for the required time as well.
The same conclusion can be drawn for Scipy's solver.
The difference between the implementations is the real time it takes to compute
the eigenvalues.
Not surprisingly, our implementation is one order magnitude slower.
